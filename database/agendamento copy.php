...
if (isset($_GET['operacao'])) {
		...
		fwrite($arquivo, 'PARAMETROS ENT' . json_decode($_POST['parametros']) . "\n");
		fclose($arquivo);

                /* $array = json_decode(json_encode($_POST['parametros']), true);
                $arquivo = fopen("C:TRADESIS/tmp/LOG.txt", "a");
		fwrite($arquivo, 'TESTES' . json_encode($array) . "\n");
		fclose($arquivo); */
                $object = (object)$_POST['parametros'];
                ...
                $arquivo = fopen("C:TRADESIS/tmp/LOG.txt", "a");
		fwrite($arquivo, 'PARAMETROS' . json_encode($parametros) . "\n");
		fclose($arquivo);
                return;
                $apiEntrada = array(
